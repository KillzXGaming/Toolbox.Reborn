...
namespace STLibrary.Forms
{
    public class Viewport2D : UserControl
    {
        ...
        {
            RenderEditor();
            var coords = OpenGL2D.OpenGLHelper.convertScreenToWorldCoords(screenMouse.X, screenMouse.Y);
            GL.PopMatrix();
        ...
        private void glControl1_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (!showSelectionBox && isPicked)
            {
                ...
                RenderEditor();
                var temp = e.Location;
                var curPos = OpenGL2D.OpenGLHelper.convertScreenToWorldCoords(temp.X, temp.Y);
                var prevPos = OpenGL2D.OpenGLHelper.convertScreenToWorldCoords(pickOriginMouse.X, pickOriginMouse.Y);
